...
inline void Raft::start_server() {
  ...
  auto grpcServiceHandler = std::make_unique<RaftServiceHandler>(this);
  // RaftServiceHandler raft_service_handler(this);
  builder.RegisterService(grpcServiceHandler.get()); /* (replace nullptr) */

  std::unique_ptr<Server> server(builder.BuildAndStart());
  logger->info("Raft server {} listening on {}", id, listening_addr);

  this->server_ = std::move(server);
  this->grpcServiceHandler_ = std::move(grpcServiceHandler);
